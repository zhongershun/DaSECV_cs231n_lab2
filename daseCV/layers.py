...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_hat = (x - sample_mean) / np.sqrt(sample_var + eps) ## 归一化输入数据
        
#         print("x_hat.shape==x.shape: ",x_hat.shape==x.shape)
        
        out = gamma * x_hat + beta
        cache = (x, gamma, beta, x_hat, sample_mean, sample_var, eps)
        
        running_men = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        pass
...
def batchnorm_backward(dout, cache):
    ...
    ## print(gamma*dout==dout*gamma)
    dx_hat = gamma*dout
    ## print(dx_hat.shape)
    
    dx_hatx_mu = np.sum((x - sample_mean) * dx_hat, axis = 0)
    
    dx_hatx_sigma = ((sample_var+ eps) ** -0.5) * dx_hat
    
    dx_sigma = (-0.5) * ((sample_var + eps) ** -1.5) * dx_hatx_mu
    
    dx_mu = dx_sigma * 1
    
    dx_mu_x = np.ones_like(x) / N * dx_mu
    
    dx_sigma_x = 2 * (x - sample_mean) * dx_mu_x
    
    dx_frac1 = dx_sigma_x * 1 + dx_hatx_sigma * 1
    
    dx_frac1_tmp = dx_sigma_x * 1 + dx_hatx_sigma * 1
    
    dx_sum = -1 * np.sum(dx_frac1_tmp, axis = 0)
    
    dx_frac0 = np.ones_like(x) / N * dx_sum
    
    dx = dx_frac0 + dx_frac1
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    x_T_hat = (x_T - sample_mean) / np.sqrt(sample_var + eps) ## 归一化输入数据
...
def layernorm_backward(dout, cache):
    ...
    dvar = (dxhat_T * (x_T-sample_mean) * (-0.5) * np.power(sample_var+eps, -1.5)).sum(axis = 0) 
    dmean = np.sum(dxhat_T * (-1) * np.power(sample_var + eps, -0.5), axis = 0)
    dmean += dvar * np.sum(-2 * (x_T - sample_mean), axis = 0) / N
    dx_T = dxhat_T * np.power(sample_var + eps, -0.5) + dvar*2*(x_T - sample_mean) / N + dmean / N
    dx = dx_T.T
    pass
