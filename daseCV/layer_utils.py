...
def affine_relu_forward(x, w, b):
    ...
    """
    a, fc_cache = affine_forward(x, w, b)  ## a = out,fc_cache = cahce
    out, relu_cache = relu_forward(a)    ## out = relu_out,relu_cache = Relu(out)
    cache = (fc_cache, relu_cache)      ## cache = (fc_cache,relu_cache)
    return out, cache
...
def affine_relu_backward(dout, cache):
    ...
    da = relu_backward(dout, relu_cache)      ## 先用dout计算relu函数的梯度
    dx, dw, db = affine_backward(da, fc_cache)  ## 将上一步计算的relu梯度作为新的dout带入反向传播计算
    return dx, dw, db
