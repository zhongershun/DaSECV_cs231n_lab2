...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        self.params['W1'] = weight_scale * np.random.randn(F, C, HH, WW) ## 卷积层
        self.params['W2'] = weight_scale * np.random.randn(int(F*H/2*W/2), hidden_dim) ## 隐藏层
        self.params['W3'] = weight_scale * np.random.randn(hidden_dim, num_classes) ## 输出层
    ...
    def loss(self, X, y=None):
        ...
        pool_out, pool_cache = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param) ## 卷积层操作
        affine_out, affine_cache = affine_relu_forward(pool_out, W2, b2) ## Relu仿射操作
        
        scores, cache = affine_forward(affine_out, W3, b3) ## 输出
        ...
        loss += 0.5 * self.reg * (np.sum(W1 ** 2) + np.sum(W2 ** 2) + np.sum(W3 ** 2)) ## 正则化
